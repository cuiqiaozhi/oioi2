...
exports.install = exports.getCommandNames = exports.enableHelp = void 0;
...
function formatCommands(path, session, children, options) {
    ...
    const hints = [];
    if (options.authority)
        hints.push((0, utils_1.template)('internal.hint-authority'));
    if (hasSubcommand)
        hints.push((0, utils_1.template)('internal.hint-subcommand'));
    output.unshift((0, utils_1.template)(path, [utils_1.template.brace(hints)]));
    return output;
}
...
function getOptions(command, session, config) {
    ...
    const output = config.authority && options.some(o => o.authority)
        ? [(0, utils_1.template)('internal.available-options-with-authority')]
        : [(0, utils_1.template)('internal.available-options')];
...
async function showHelp(command, session, config) {
    ...
    if (command._aliases.length > 1) {
        output.push((0, utils_1.template)('internal.command-aliases', Array.from(command._aliases.slice(1)).join('，')));
    }
    output.push(...getOptions(command, session, config));
    if (command._examples.length) {
        output.push((0, utils_1.template)('internal.command-examples'), ...command._examples.map(example => '    ' + example));
    }
    output.push(...formatCommands('internal.subcommand-prolog', session, command.children, config));
    return output.filter(Boolean).join('\n');
}
/* eslint-disable quote-props */
utils_1.template.set('internal', {
    // command
    'low-authority': '权限不足。',
...
function install(ctx) {
    ...
    ctx.command('help [command]', '显示帮助信息')
        ...
        if (!target) {
            ...
            const epilog = (0, utils_1.template)('internal.global-help-epilog');
            if (epilog)
                output.push(epilog);
            return output.filter(Boolean).join('\n');
        }
